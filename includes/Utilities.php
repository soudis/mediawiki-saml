...
class MoSamlUtilities {
	...
	public static function createAuthnRequest($acsUrl, $issuer, $destination, $force_authn = 'false', $sso_binding_type = 'HttpRedirect') {
	...
	public static function validateIssuerAndAudience($samlResponse, $spEntityId, $issuerToValidateAgainst) {
		...
		if(strcmp($issuerToValidateAgainst, $issuer) === 0) {
			if(strcmp($audience, $spEntityId) === 0) {
				...
				echo 'Invalid audience URI. Expected '. $spEntityId . ", found " . $audience;
				return false;
			}
		} else {
	...
	private static function findCertificate(array $certFingerprints, array $certificates) {
		...
		echo 'Unable to find the configured certificate in the SAML Response.';
		exit;
	}
