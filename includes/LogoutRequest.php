...
class SAML2_LogoutRequest
{
    ...
    public function __construct(DOMElement $xml = NULL)
    {
        ...
        if ($xml->hasAttribute('NotOnOrAfter')) {
            $this->notOnOrAfter = MoSamlUtilities::xsDateTimeToTimestamp($xml->getAttribute('NotOnOrAfter'));
        }

        $nameId = MoSamlUtilities::xpQuery($xml, './saml_assertion:NameID | ./saml_assertion:EncryptedID/xenc:EncryptedData');
        if (empty($nameId)) {
        ...
        $sessionIndexes = MoSamlUtilities::xpQuery($xml, './saml_protocol:SessionIndex');
