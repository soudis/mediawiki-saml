...
class SAML2_Assertion
{
    ...
    {
        $this->id = MoSamlUtilities::generateId();
        $this->issueInstant = MoSamlUtilities::generateTimestamp();
        $this->issuer = '';
        $this->authnInstant = MoSamlUtilities::generateTimestamp();
		...
		if($xml->localName === 'EncryptedAssertion'){
				$data = MoSamlUtilities::xpQuery($xml, './xenc:EncryptedData');
				$encryptedMethod =  MoSamlUtilities::xpQuery($xml, './xenc:EncryptedData/ds:KeyInfo');
				$method = $encryptedMethod[0]->firstChild->firstChild->getAttribute("Algorithm");
				$algo = MoSamlUtilities::getEncryptionAlgorithm($method);
				if (count($data) === 0) {
				...
				//$url = plugins_url('resources/sp-key.key', __FILE__);
				...
				//$alternateKeyUrl = plugins_url('resources/miniorange_sp_priv_key.key', __FILE__);
				$alternateKey->loadKey($alternateKeyUrl,TRUE);
				
				$blacklist = array();
				$xml = MoSamlUtilities::decryptElement($data[0], $key, $blacklist, $alternateKey);
		}
        ...
        $this->issueInstant = MoSamlUtilities::xsDateTimeToTimestamp($xml->getAttribute('IssueInstant'));
		
        $issuer = MoSamlUtilities::xpQuery($xml, './saml_assertion:Issuer');
        if (empty($issuer)) {
    ...
    {
        $subject = MoSamlUtilities::xpQuery($xml, './saml_assertion:Subject');
        if (empty($subject)) {
            /* No Subject node. */
        ...
        $nameId = MoSamlUtilities::xpQuery(
            $subject,
        ...
        if ($nameId->localName === 'EncryptedData') {
            ...
            $this->nameId = MoSamlUtilities::parseNameId($nameId);
        }
		//echo 'AssertionNameID: '. $this->nameId['Value'];
        /*$subjectConfirmation = MoSamlUtilities::xpQuery($subject, './saml_assertion:SubjectConfirmation');
    ...
    {
        $conditions = MoSamlUtilities::xpQuery($xml, './saml_assertion:Conditions');
        if (empty($conditions)) {
        ...
        if ($conditions->hasAttribute('NotBefore')) {
            $notBefore = MoSamlUtilities::xsDateTimeToTimestamp($conditions->getAttribute('NotBefore'));
            if ($this->notBefore === NULL || $this->notBefore < $notBefore) {
                $this->notBefore = $notBefore;
            }
        }
        if ($conditions->hasAttribute('NotOnOrAfter')) {
            $notOnOrAfter = MoSamlUtilities::xsDateTimeToTimestamp($conditions->getAttribute('NotOnOrAfter'));
        ...
        for ($node = $conditions->firstChild; $node !== NULL; $node = $node->nextSibling) {
            ...
            switch ($node->localName) {
                case 'AudienceRestriction':
                    $audiences = MoSamlUtilities::extractStrings($node, 'urn:oasis:names:tc:SAML:2.0:assertion', 'Audience');
    ...
    {
        $authnStatements = MoSamlUtilities::xpQuery($xml, './saml_assertion:AuthnStatement');
        ...
        $this->authnInstant = MoSamlUtilities::xsDateTimeToTimestamp($authnStatement->getAttribute('AuthnInstant'));

        if ($authnStatement->hasAttribute('SessionNotOnOrAfter')) {
            $this->sessionNotOnOrAfter = MoSamlUtilities::xsDateTimeToTimestamp($authnStatement->getAttribute('SessionNotOnOrAfter'));
        }
    ...
    private function parseAuthnContext(DOMElement $authnStatementEl)
    {
        ...
        $authnContexts = MoSamlUtilities::xpQuery($authnStatementEl, './saml_assertion:AuthnContext');
        ...
        $authnContextDeclRefs = MoSamlUtilities::xpQuery($authnContextEl, './saml_assertion:AuthnContextDeclRef');
        ...
        $authnContextDecls = MoSamlUtilities::xpQuery($authnContextEl, './saml_assertion:AuthnContextDecl');
        ...
        $authnContextClassRefs = MoSamlUtilities::xpQuery($authnContextEl, './saml_assertion:AuthnContextClassRef');
        ...
        $this->AuthenticatingAuthority = MoSamlUtilities::extractStrings(
            $authnContextEl,
    ...
    {
        $firstAttribute = TRUE;
        $attributes = MoSamlUtilities::xpQuery($xml, './saml_assertion:AttributeStatement/saml_assertion:Attribute');
        foreach ($attributes as $attribute) {
            ...
            $values = MoSamlUtilities::xpQuery($attribute, './saml_assertion:AttributeValue');
    ...
    {
        $this->encryptedAttribute = MoSamlUtilities::xpQuery(
            $xml,
    ...
    private function parseSignature(DOMElement $xml)
    {
        ...
        $sig = MoSamlUtilities::validateElement($xml);
        if ($sig !== FALSE) {
    ...
    public function validate(XMLSecurityKey $key)
    {
        ...
        MoSamlUtilities::validateSignature($this->signatureData, $key);
     ...
     *
     * The returned NameId is in the format used by MoSamlUtilities::addNameId().
     *
     * @see MoSamlUtilities::addNameId()
     ...
     *
     * The NameId must be in the format accepted by MoSamlUtilities::addNameId().
     *
     * @see MoSamlUtilities::addNameId()
    ...
    public function encryptNameId(XMLSecurityKey $key)
    {
        ...
        MoSamlUtilities::addNameId($root, $this->nameId);
        $nameId = $root->firstChild;

        MoSamlUtilities::getContainer()->debugMessage($nameId, 'encrypt');
    ...
    public function decryptNameId(XMLSecurityKey $key, array $blacklist = array())
    {
        ...
        $nameId = MoSamlUtilities::decryptElement($this->encryptedNameId, $key, $blacklist);
        MoSamlUtilities::getContainer()->debugMessage($nameId, 'decrypt');
        $this->nameId = MoSamlUtilities::parseNameId($nameId);
    ...
    public function decryptAttributes(XMLSecurityKey $key, array $blacklist = array())
    {
        ...
        foreach ($attributes as $attributeEnc) {
            ...
            $attribute = MoSamlUtilities::decryptElement(
    ...
    public function toXML(DOMNode $parentElement = NULL)
    {
        ...
        $issuer = MoSamlUtilities::addString($root, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:Issuer', $this->issuer);
        ...
        if ($this->signatureKey !== NULL) {
            MoSamlUtilities::insertSignature($this->signatureKey, $this->certificates, $root, $issuer->nextSibling);
        }
    ...
    private function addSubject(DOMElement $root)
    {
        ...
        if ($this->encryptedNameId === NULL) {
            MoSamlUtilities::addNameId($subject, $this->nameId);
        } else {
    ...
    private function addConditions(DOMElement $root)
    {
        ...
        if ($this->validAudiences !== NULL) {
            ...
            MoSamlUtilities::addStrings($ar, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:Audience', FALSE, $this->validAudiences);
        }
    }
